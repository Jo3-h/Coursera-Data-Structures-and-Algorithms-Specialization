...
public:
    ...
    Response Process(const Request &request) {
        ...
        //if there are zero processing then add request and return
        ...
        //remove any front elements that have finished processing
        while (finish_time_.size() > 0 && finish_time_.front() <= request.arrival_time){
            finish_time_.pop();
        }
        
        //if there is room in the buffer, add to the back. Update finish
        if (finish_time_.size() == 0){
